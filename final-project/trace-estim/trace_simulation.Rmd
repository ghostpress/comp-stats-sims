...
stddev_list       <- vector(mode="list", length=length(N))
...
for(n in N) {
  ...
  stddev_list[index]    <- sqrt(as.double(trace_est$S))
...
```{r}
err_data   <- data.frame(N, t(abs_err), rms_err, t(t(trace_est_list)), t(t(stddev_list)))
trace_data <- data.frame(N, t(t(trace_est_list)), t(t(trace_true_list)))
run_data   <- data.frame(N, t(t(runtime_est_list)), t(t(runtime_true_list)))
