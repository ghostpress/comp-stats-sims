...
schattenEstimate <- function(B, p, k) {
  ...
  O <- drawTestM(n, k)  # draw the test matrix
  Y <- B %*% O          # compute the sample matrix
  X <- t(Y) %*% Y       # compute the Gram matrix  FIXME: becomes large very quickly
  ...
  V <-(1/choose(k, p))  * tr(T_p %*% X)  # FIXME: use R's base function
  return(V)
}
```
...
#trace(B)
x <- schattenEstimate(B, 1, 10)  # compute the 2p = l2 norm of B
